...
impl DatabaseServer {
    ...
    pub fn finalized(mut self) -> Self {
        self.finalizers_mut().push(DATABASE_SERVER_FINALIZER.to_string());
        self
    }

    /// Modify a document to have an expected status
    pub fn with_status(mut self, status: DatabaseServerStatus) -> Self {
...
pub enum DatabaseServerScenario {
    ...
    StatusPatch(DatabaseServer),
    /// finalized objects with hide set causes both an event and then a hide patch
    EventPublishThenStatusPatch(String, DatabaseServer),
...
impl ApiServerVerifier {
    ...
    async fn handle_finalizer_creation(mut self, doc: DatabaseServer) -> Result<Self> {
        ...
        let expected_patch = serde_json::json!([
            ...
            { "op": "add", "path": "/metadata/finalizers", "value": vec![DATABASE_SERVER_FINALIZER] }
        ]);
    ...
    async fn handle_finalizer_removal(mut self, doc: DatabaseServer) -> Result<Self> {
        let (request, send) = self.0.next_request().await.expect("service not called");
        ...
        let expected_patch = serde_json::json!([
            { "op": "test", "path": "/metadata/finalizers/0", "value": DATABASE_SERVER_FINALIZER },
    ...
    async fn handle_status_patch(mut self, doc: DatabaseServer) -> Result<Self> {
        let (request, send) = self.0.next_request().await.expect("service not called");
        assert_eq!(request.method(), http::Method::PATCH);
        assert_eq!(
            request.uri().to_string(),
        ...
        let status: DatabaseServerStatus = serde_json::from_value(status_json).expect("valid status");
        assert_eq!(
            status.enabled, doc.spec.enable,
            "status.enabled iff doc.spec.enable"
        );
